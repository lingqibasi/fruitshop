...
<body>
<form action="${pageContext.request.contextPath}/assets/iuser/likeSelect" method="POST">
...
<%--        </div>--%>
        <div id="name" style="margin-right:20px">
            <lable for="name" class="layui-form-label required">用户名</lable>
...
<table class="layui-table" lay-filter="test">
    ...
    <c:forEach items="${iusers.list}" var="iuser">
        <tr align="center">
...
<%--            <td>${sys_user.age}</td>--%>
            <td class="create">${iuser.createtime}</td>
            <td class="update">${iuser.updatetime}</td>
            <td><a href="${pageContext.request.contextPath}/assets/iuser/toDetail?id=${iuser.id}"
                   class="layui-btn layui-btn-xs" lay-event="edit">修改</a> <a
                    href="${pageContext.request.contextPath}/assets/iuser/delete?id=${iuser.id}"
                    onclick="return confirm('确定将编号为${iuser.id}记录删除?')"
...
<script>
    layui.use(['form', 'jquery', 'layer','laypage'], function () {
        ...
        var pageNum = ${iusers.pageNum};
        //自定义样式
        laypage.render({
            elem: 'demo2',
            count: ${iusers.total},
            limit: ${iusers.pageSize},
            curr: pageNum,
            theme: '#1E9FFF',
